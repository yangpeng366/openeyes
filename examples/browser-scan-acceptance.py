"""Run the two-tab browser_scan read-only acceptance over direct MCP stdio.

This probe extends the repository-local surrogate coverage to the read-only
``browser_scan`` URL-filter contract. It mirrors
``browser-type-acceptance.py``: self-contained transient HTTP fixture server,
two disposable tabs via the CDP ``/json/new`` endpoint, then a matched-scan +
fail-closed check over MCP stdio.

A matched ``url_contains`` scan must resolve elements only from the matching
target-a tab; a second scan scoped to the decoy filter must resolve the same
``Learn more`` link on the decoy tab (proving the filter selects tabs, not
element absence); and an unmatched ``url_contains`` scan must fail closed with
``no page target matched url_contains`` and return no element list.
"""
...
def _scan_element_names(payload: dict) -> list[str]:
    if not isinstance(payload, list):
        raise RuntimeError(f"browser_scan returned a non-list payload: {payload!r}")
    return [e.get("name") or "" for e in payload]
...
def main() -> int:
    ...
    try:
        ...
        decoy_filter = f"{DECOY_MARKER}.html?probe={probe_id}"
        missing_filter = f"missing-{probe_id}"
        target_url = (
            "http://127.0.0.1:"
            f"{http_port}/acceptance-pages/{TARGET_MARKER}.html?probe={probe_id}"
        )
        decoy_url = (
            "http://127.0.0.1:"
            f"{http_port}/acceptance-pages/{DECOY_MARKER}.html?probe={probe_id}"
        )
        ...
        call(
            ...
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {
                    "name": "openeyes-browser-scan-acceptance",
                    "version": "1",
                },
            },
        )
        ...
        if "browser_tabs" not in tool_names or "browser_scan" not in tool_names:
            raise RuntimeError(
                "MCP surface does not expose browser_tabs/browser_scan"
            )
        ...
        decoy_tabs = [
            tab for tab in acceptance_tabs
            if decoy_filter in (tab.get("url") or "")
        ]
        ...
        matched = _result_content(call(4, "tools/call", {
            ...
            "arguments": {
                ...
                "name_contains": "Secondary action",
            },
        }))
        matched_names = _scan_element_names(matched)
        if matched_names != ["Secondary action"]:
            raise RuntimeError(
                "matched scan did not isolate the target-a Secondary action: "
                f"{matched!r}"
            )

        decoy = _result_content(call(5, "tools/call", {
            "name": "browser_scan",
            "arguments": {
                "port": args.cdp_port,
                "url_contains": decoy_filter,
                "name_contains": "Learn more",
            },
        }))
        decoy_names = _scan_element_names(decoy)
        if decoy_names != ["Learn more"]:
            raise RuntimeError(
                "decoy scan did not isolate the decoy Learn more link: "
                f"{decoy!r}"
            )
        ...
        if isinstance(missing, list) and len(missing) > 0:
            raise RuntimeError(f"missing call returned elements: {missing!r}")
    ...
    print(json.dumps({
        ...
        "decoy_url": decoy_tabs[0]["url"],
        "matched_scan": matched,
        "decoy_scan": decoy,
        "missing_scan_error": error,
    }, ensure_ascii=False))
    return 0
